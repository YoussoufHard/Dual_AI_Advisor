-- =====================================================
-- MIGRATION COMPLÈTE - RESET ET RECONSTRUCTION
-- =====================================================
...
-- Nettoyer la table users (garder seulement les colonnes auth de Supabase)
DO $$
DECLARE
    col_name text;
    cols_to_drop text[] := ARRAY[
        'full_name', 'name', 'avatar_url', 'current_role_title', 'target_role_title',
        'years_experience', 'experience_level', 'industry', 'goals', 'skills',
        'interests', 'preferences', 'metadata', 'last_active_at', 'is_active',
        'location', 'github_url', 'linkedin_url', 'portfolio_url', 'subscription_tier'
    ];
BEGIN
    FOREACH col_name IN ARRAY cols_to_drop LOOP
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = col_name) THEN
...
-- =====================================================
-- RECONSTRUCTION DE LA TABLE USERS
-- =====================================================
...
-- Triggers pour updated_at (créés APRÈS les tables)
...
-- Créer une vue matérialisée pour les analytics du dashboard
...
-- Commentaire final
COMMENT ON SCHEMA public IS 'Plateforme AI/ML/Data/Cloud - Base de données complète avec analytics temps réel, ML, et infrastructure cloud';