-- =====================================================
-- MIGRATION COMPLÈTE AVEC STRIPE - VERSION CORRIGÉE
-- =====================================================
...
-- Ajouter les contraintes après avoir ajouté les colonnes
DO $$
BEGIN
  -- Contrainte pour experience_level
  IF NOT EXISTS (SELECT 1 FROM information_schema.check_constraints WHERE constraint_name = 'users_experience_level_check') THEN
    ALTER TABLE users ADD CONSTRAINT users_experience_level_check 
    CHECK (experience_level IN ('student', 'junior', 'mid', 'senior', 'lead'));
  END IF;
  
  -- Contrainte pour subscription_tier
  IF NOT EXISTS (SELECT 1 FROM information_schema.check_constraints WHERE constraint_name = 'users_subscription_tier_check') THEN
...
-- RLS Policies for users (nouvelles politiques)
...
CREATE POLICY "chat_sessions_manage_own" ON chat_sessions
  FOR ALL TO authenticated
  USING (user_id = auth.uid());

-- RLS Policies for skill_assessments
CREATE POLICY "skill_assessments_manage_own" ON skill_assessments
  FOR ALL TO authenticated
  USING (user_id = auth.uid());

-- RLS Policies for analytics_events
CREATE POLICY "analytics_events_insert_tracking" ON analytics_events
  FOR INSERT TO authenticated
  WITH CHECK ((auth.uid() = user_id) OR (user_id IS NULL));

CREATE POLICY "analytics_events_insert_anonymous" ON analytics_events
  FOR INSERT TO anon
...
CREATE POLICY "job_market_data_read_all" ON job_market_data
  FOR SELECT TO authenticated
  USING (true);

-- RLS Policies for learning_progress
CREATE POLICY "learning_progress_manage_own" ON learning_progress
  FOR ALL TO authenticated
  USING (user_id = auth.uid());
...
CREATE POLICY "recommendations_manage_own" ON recommendations
  FOR ALL TO authenticated
  USING (user_id = auth.uid());

-- RLS Policies for user_interactions
CREATE POLICY "user_interactions_manage_own" ON user_interactions
  FOR ALL TO authenticated
  USING (user_id = auth.uid());

-- RLS Policies for ml_predictions
CREATE POLICY "ml_predictions_manage_own" ON ml_predictions
  FOR ALL TO authenticated
  USING (user_id = auth.uid());

-- RLS Policies for chat_conversations
CREATE POLICY "chat_conversations_manage_own" ON chat_conversations
  FOR ALL TO authenticated
  USING (user_id = auth.uid());

-- Public read access for reference data
CREATE POLICY "skill_classifications_read_public" ON skill_classifications
  FOR SELECT TO authenticated
  USING (true);

CREATE POLICY "market_data_read_public" ON market_data
  FOR SELECT TO authenticated
  USING (true);

-- Admin policies for cloud metrics
CREATE POLICY "cloud_metrics_read_admin" ON cloud_metrics
  FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM users 
