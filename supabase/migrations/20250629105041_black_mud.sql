/*
  # Solution Complète - Reset et Reconstruction de la Base de Données
  
  Cette migration offre plusieurs stratégies :
  1. Reset complet de la base de données
  2. Gestion intelligente des conflits
  3. Création d'une nouvelle base propre
  
  Choisissez la stratégie qui convient le mieux à votre situation.
*/
...
DO $$
BEGIN
  -- Supprimer toutes les colonnes ajoutées, garder seulement les colonnes auth de base
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'avatar_url') THEN
    ALTER TABLE users DROP COLUMN avatar_url CASCADE;
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'role_title') THEN
    ALTER TABLE users DROP COLUMN role_title CASCADE;
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'years_experience') THEN
    ALTER TABLE users DROP COLUMN years_experience CASCADE;
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'industry') THEN
    ALTER TABLE users DROP COLUMN industry CASCADE;
  END IF;
  ...
  ALTER TABLE users DROP CONSTRAINT IF EXISTS users_goals_check;
...
ALTER TABLE users ADD COLUMN experience_level text NOT NULL DEFAULT 'student' 
...
ALTER TABLE users ADD COLUMN current_role_title text;
ALTER TABLE users ADD COLUMN target_role_title text;
