/*
  # Enhanced AI/ML/Data/Cloud Platform Database Schema

  1. New Tables
    - `users` - Enhanced user profiles with ML features
    - `user_sessions` - Session tracking for analytics
    - `recommendations` - AI/ML recommendation storage
    - `user_interactions` - Detailed interaction tracking
    - `ml_predictions` - ML model predictions and results
    - `analytics_events` - Real-time analytics events
    - `skill_classifications` - ML-classified skills database
    - `market_data` - Job market insights and trends
    - `cloud_metrics` - Infrastructure monitoring
    - `chat_conversations` - Chat history and analytics

  2. Security
    - Enable RLS on all user tables
    - Comprehensive policies for data access
    - Admin controls for sensitive data

  3. Performance
    - Optimized indexes including GIN for JSONB
...
BEGIN
  ...
  DROP POLICY IF EXISTS "Users can insert own predictions" ON ml_predictions;
  ...
  DROP POLICY IF EXISTS "Allow anonymous analytics events" ON analytics_events;
  ...
  DROP POLICY IF EXISTS "Public read access for skill classifications" ON skill_classifications;
  DROP POLICY IF EXISTS "Public read access for market data" ON market_data;
  DROP POLICY IF EXISTS "Admin read access for cloud metrics" ON cloud_metrics;
EXCEPTION
